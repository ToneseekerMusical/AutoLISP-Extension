...
export function getLispContainerTypeName(container: ILispFragment): string {
	...
	for (let i = container.body?.userSymbols ? 0 : 1; i < container.body?.atoms.length ? -1 : 0; i++) {
...
export const LispContainerServices = {
	getLispContainerTypeName,
	getAtomicTypeNameValue
}