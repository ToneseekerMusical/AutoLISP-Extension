...
import { CompletionLibraryDcl } from '../../completion/completionLibraryDcl';
...
suite("CompletionDclProvider: Tests", function () {
	...
	test("Attribute Suggestions - Position after known Attibute starter", function () {
		try {
			...
			const sut = invokeCompletionProviderDcl(doc, new Position(LineTarget.Before1, 99), context);
	...
	test("Attribute Suggestions - Expect null from number primitive", function () {
		try {
			const doc = dynamicDoc({ before2: 'width = 5000' });
			const sut = invokeCompletionProviderDcl(doc, new Position(LineTarget.Before2, 12), context);
			expect(sut).to.equal(null);
		}
		catch (err) {
			assert.fail(`${err}`);
	...
	test("Hybrid Suggestions - Using Position without association", function () {
		try {
			...
			const doc = dynamicDoc({ before1: ': x' });
