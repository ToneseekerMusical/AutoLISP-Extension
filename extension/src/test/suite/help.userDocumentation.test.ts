...
const lspFilePath = path.join(prefixPath + "pdfMarkups.lsp");
...
suite("Help: UserDocumentation Tests", function () {
	...
	suiteSetup(async () => {
		try {
			...
			console.log(`Failed to setup tests for user documentation. This is likely due to the test file not being found or parsed correctly. Please ensure the test file exists and is correctly formatted.\n${err}`);
