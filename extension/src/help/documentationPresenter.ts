import { WebHelpDclAtt, WebHelpEntity, WebHelpDclTile, WebHelpFunction, WebHelpValueType } from './documentationObjects';
...
function webLink(text: string, url: string): string {
	return `[${text}](${url})`;
}
//eslint-disable-next-line
function fileLink(text: string, filePath: string, line: number): string {
	return text;
...
function addParam(source: WebHelpValueType, active: boolean): string {
	return `${divider}\n${webHelpValueTypeHandler(source, italic('param'), active)}`;
}

function addReturn(source: WebHelpValueType): string {
...
const MarkdownHelpers = {
	...
	addParam,
	addReturn,
	webHelpValueTypeHandler
}

// TODO: later versions of VSCode.MarkdownString allow HTML and would give us a lot more layout/presentation control

function asMarkdown(source: string): vscode.MarkdownString;
function asMarkdown(source: LispAtom, paramIndex?: number, args?: LispAtom[], docs?: ILispDocs, path?: string): vscode.MarkdownString;
function asMarkdown(source: WebHelpFunction, paramIndex?: number): vscode.MarkdownString;
function asMarkdown(source: WebHelpDclAtt): vscode.MarkdownString;
function asMarkdown(source: WebHelpDclTile): vscode.MarkdownString;
function asMarkdown(source: WebHelpEntity): vscode.MarkdownString;
function asMarkdown(source: string | LispAtom | WebHelpEntity,
...
function FunctionMarkdown(source: WebHelpFunction, paramIndex: number): vscode.MarkdownString {
	const lines = [];
	const year = AutoLispExt.WebHelpLibrary.year;
	const url = source.getHelpLink(year);
...
function extractDynamicSignature(source: LispAtom, args: LispAtom[], index: number, docs: ILispDocs, lines: string[]) {
	let sig = `(${source.symbol}`;
	const expect = args.map(x => x.symbol.toLowerCase());
	for (let i = 0; i < docs.params.length; i++) {
		const param = docs.params[i];

		const id = args.length < i ? args[i].symbol : param.name;
