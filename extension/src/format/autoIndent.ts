...
export function isCursorInDoubleQuoteExpr(document: vscode.TextDocument, position: vscode.Position): boolean {
    try {
        const topContainer = findContainers(document, position).containerParens.pop();

        const startPos2d = document.positionAt(topContainer.startPos.offsetInDocument);
        const endPos2d = document.positionAt(topContainer.endPos.offsetInDocument + 1);
        ...
        readerStartPos.offsetInDocument = topContainer.startPos.offsetInDocument; //the start position in doc
        const reader = new format.ListReader(sexpr, readerStartPos, document);

        const atomAtCursor = reader.tokenize().getAtomFromPos(position);
        return atomAtCursor != null && atomAtCursor.symbol.startsWith("\"") ? true : false
    } catch (err) {
        console.log(err);
        return false;
    }
...
function getTabSize(): number {
    ...
    return editor ? +editor.options.tabSize : 2;
}
...
function getNumber_Defun(document: TextDocument, cursorPos2d: Position, semantics: BasicSemantics): number {
    ...
    switch (true) {
        case semantics.operator == null || !semantics.operatorLowerCase.includes("defun"):
            return -1
        case semantics.operands.length == 0 || isPosBetween(cursorPos2d, operator.line, operator.column, funName.line, funName.column):
            //assuming user is adding function name, which should be after, e.g., "defun "
            return character2Column(operator.column, operator.line, document) + operator.symbol.length + 1;
        case semantics.operands.length == 1 || isPosBetween(cursorPos2d, funName.line, funName.column, argList.line, argList.column):
            ...
            return character2Column(funName.column, funName.line, document);
        default: return -1
    }
}
...
function getWhiteSpaceNumber(document: TextDocument, exprInfo: ElementRange, parentParenExpr: ElementRange, cursorPos2d: Position): number {
