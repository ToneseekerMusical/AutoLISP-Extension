...
export class ProjectDefinition {
	...
	public static CreateEmpty(prjName: string): ProjectDefinition {
		...
		const ret = new ProjectDefinition();
	...
	public static Create(prjExpr: Sexpression): ProjectDefinition {
		...
		const atomNum = prjExpr.atoms.length;
		if (atomNum % 2 != 1) {
		...
		const pairNum = (atomNum - 1) / 2 - 1;
		for (let i = 0; i < pairNum; i++) {
			const keyIndex = 2 + i * 2;
			const valueIndex = 2 + i * 2 + 1;

			const key = prjExpr.atoms[keyIndex].symbol.toUpperCase();
	...
	public get Name(): string {
		if (!Object.prototype.hasOwnProperty.call(this.metaData, ProjectDefinition.key_name))
			return null;
	...
	public static isStandardProperty(key: string): boolean {
		...
		for (const sKey of ProjectDefinition.standardKeys) {
			if (upper == sKey)
				return true;
		}
