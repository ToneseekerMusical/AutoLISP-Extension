...
export class ProjectNode implements DisplayNode {
	...
	getDisplayText(): string {
		if (this.projectModified) {
			const unsaved = localize("autolispext.project.tree.unsaved", " (UNSAVED)");
...
export class LspFileNode implements DisplayNode {
	...
	getTooltip(): string {
		...
		if (this.fileExists) {
			...
			const msg = localize("autolispext.project.tree.filenotexist", "File doesn't exist: ");
	...
	public getChildren(element?: DisplayNode): vscode.ProviderResult<DisplayNode[]> {
		try {
			...
			if (!element) {
				...
				const ret: DisplayNode[] = [this.rootNode];
				return ret;
			}
...
export function hasFileWithSameName(fileName: string, root: ProjectNode): boolean {
	const lowerLeft = path.basename(fileName).toLocaleLowerCase();

	for (const fileNode of root.sourceFiles) {
		const lowerRight = path.basename(fileNode.filePath).toLocaleLowerCase();
...
export function addLispFileNode2ProjectTree(root: ProjectNode, fileName: string, rawFilePath: string) {
	const fileNode = new LspFileNode();
