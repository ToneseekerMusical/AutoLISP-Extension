...
export class ReadonlyDocument implements vscode.TextDocument {
	...
	static open(filePath: string): ReadonlyDocument {
		...
		const ret = new ReadonlyDocument(filePath);
	...
	static createMemoryDocument(fileContent: string, languageId: string): ReadonlyDocument {
		const ret = new ReadonlyDocument('');
	...
	offsetAt(position: vscode.Position): number {
		...
		//now the last line
		const lastLine = this.lines[position.line];
	...
	positionAt(offset: number): vscode.Position {
		...
		for (let line = 0; line < this.lineCount; line++) {
			...
			if (charCounted + lineText.length >= offset) {
				...
				const charNum = offset - charCounted;
		...
		const msg = localize("autolispext.project.readonlydocument.convertoffsettopositionfailed", "Failed to convert offset to position.");
		throw new Error(msg);
	}
	...
	get documentContainer(): LispContainer {
		if (this.languageId !== Selectors.LSP) {
			return null;
		}
	...
	get documentDclContainer(): DclTile {
		if (this.languageId !== Selectors.DCL) {
			return null;
		}
	...
	get isLSP(): boolean {
		return this.languageId === Selectors.LSP;
	}
}