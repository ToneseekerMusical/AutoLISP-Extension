...
export async function excludeFromProject(selected: LspFileNode) {
    if (ProjectTreeProvider.hasProjectOpened() == false) {
        const msg = localize("autolispext.project.excludefile.openproject", "A project must be open before you can exclude a file.");
        return Promise.reject(msg);
    }

    let index2Del = -1;
    for (let i = 0; i < ProjectTreeProvider.instance().projectNode.sourceFiles.length; i++) {
        const fileNode = ProjectTreeProvider.instance().projectNode.sourceFiles[i];
    ...
    if (index2Del < 0) {
        const msg = localize("autolispext.project.excludefile.filenotexist", "File to exclude doesn't exist in the current project.");
        return Promise.reject(msg);
    }
