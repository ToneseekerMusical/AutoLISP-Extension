	...
	try {
		...
		for (let lineNum = 0; lineNum >= 0; lineNum++) {
			...
			if (newlinePos < 0) {
				...
				if (isCrLf) {
					newFileContent += chooseLineContent(lineNum, data.substring(0, newlinePos - 1), filePlan);
				}
				else {
					newFileContent += chooseLineContent(lineNum, data.substring(0, newlinePos), filePlan);
				}
