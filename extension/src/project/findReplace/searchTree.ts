...
export class FindingNode implements DisplayNode {
	...
	getTooltip(): string {
		const tooltip = `${this.line}: ${this.text}`;
...
export class SummaryNode implements DisplayNode {
	...
	makeTooltip(opt: SearchOption, prjNode: ProjectNode) {
		...
		if (prjNode) {
			const inStr = localize("autolispext.project.findreplace.searchtree.in", " in ");
			const prjFileStr = localize("autolispext.project.findreplace.searchtree.prjfile", "\r\nProject file: ");
			prjNameClause = inStr + `${prjNode.projectName}`;
			prjPathStatement = prjFileStr + `${prjNode.projectFilePath}`;
		}

		if (opt.isReplace) {
			const replace = localize("autolispext.project.findreplace.searchtree.replace", "Replace ");
			const withStr = localize("autolispext.project.findreplace.searchtree.with", " with ");
			this.tooltip = replace + `"${opt.keyword}"` + withStr + `"${opt.replacement}"${prjNameClause};`;
		}
		else {
			const findStr = localize("autolispext.project.findreplace.searchtree.find", "Find ");
...
export class SearchTreeProvider implements vscode.TreeDataProvider<DisplayNode> {
	...
	public reset(newResult: DisplayNode[], summary: SummaryNode, opt: SearchOption) {
		...
		if (newResult != null) {
			for (const fileNode of newResult) {
	...
	public getTreeItem(element: DisplayNode): vscode.TreeItem | Thenable<vscode.TreeItem> {
		try {
			const treeNode = new vscode.TreeItem(element.getDisplayText());
		...
		try {
			if (!element) {
				...
				if (element instanceof FileNode) {
					const fileNode = element as FileNode;
