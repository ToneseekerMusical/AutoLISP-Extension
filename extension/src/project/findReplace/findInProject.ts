...
export async function findInProject() {
	...
	if (ProjectTreeProvider.hasProjectOpened() == false) {
		const msg = localize("autolispext.project.find.openproject", "A project must be open before you can search for a text string.");
		vscode.window.showInformationMessage(msg);
		return;
	}

	//get search option
	const title = localize("autolispext.project.find.title", "Find in Project");
	const hint = localize("autolispext.project.find.hint", "Type a text string to find, and press Enter.");
	const opt = await getSearchOption(title, hint);
...
export class FindInProject {
	...
	public async execute(searchOption: SearchOption, prjNode: ProjectNode) {
		if (os.platform() == 'win32') {
			if (os.arch() != 'x64') {
				const msg = localize("autolispext.project.find.supportos", "Find & Replace is supported only on 64-bit systems.");
		...
		try {
			...
			const summary = localize("autolispext.project.find.inprogress", "In progress... ");
			...
			const found = localize("autolispext.project.find.found", "Found ");
			const lines = localize("autolispext.project.find.results", " result(s) in ");
			const files = localize("autolispext.project.find.files", " file(s):");
			for (const srcFile of prjNode.sourceFiles) {
				...
				const file2Search = saveOpenDoc2Tmp(srcFile.filePath);
				try {
					const encodingName = detectEncoding(file2Search);
					...
					const findings = this.parseResult(ret.stdout, srcFile.filePath);
	...
	private async findWithEncoding(searchOption: SearchOption, file2Search: string, encoding: string = "utf8") {
		try {
			const ret = await findInFile(searchOption, file2Search, encoding);
			return ret;
		} catch (ex) {
			if (Object.prototype.hasOwnProperty.call(ex, 'stderr') && (!ex.stderr) && (ex.code == 1) && ex.failed) {
				return null;
			}
			throw ex;
		}
	...
	private async findWithLocale(searchOption: SearchOption, file2Search: string) {
		...
		const ret = await this.findWithEncoding(searchOption, file2Search, encoding);
		return ret;
	}
	...
	private parseResult(result: string, file: string) {
		...
		for (const oneLine of lines) {
			...
			const line = Number(cells[0]);
			const colInBytes = Number(cells[1]);

			cells.splice(0, 2);
			const text = cells.join(':');//it's possible that the matched line text contains ":"
