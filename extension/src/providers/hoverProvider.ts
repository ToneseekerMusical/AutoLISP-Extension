...
const handlersDCL = {
	getHoverResults(roDoc: ReadonlyDocument, position: vscode.Position): vscode.Hover {
		const parent = roDoc.documentDclContainer.getParentFrom(position);
		if (!parent) {
			return null;
		}
		const atom = parent.getAtomFromPosition(position);
		return parent.asTile ? this.processTileAtom(parent, atom) : this.processAttributeAtom(parent, atom);
	},

	processAttributeAtom(parent: IDclContainer, atom: IDclFragment): vscode.Hover {
	...
	processTileAtom(parent: IDclContainer, atom: IDclFragment): vscode.Hover {
...
const handlersLSP = {
	...
	getUserResources(lowerKey: string, doc: ReadonlyDocument, pos: vscode.Position): vscode.MarkdownString {
		return this.tryLocalUserResource(lowerKey, doc, pos) ?? this.tryGlobalUserResources(lowerKey, doc);
	},

	//eslint-disable-next-line
	tryLocalUserResource(lowerKey: string, roDoc: ReadonlyDocument, pos: vscode.Position): vscode.MarkdownString {
		...
		if (defs.length === 1) {
			return this.getUserResourceMarkdown(defs[0], roDoc.documentContainer.flatten(), roDoc);
		}

		// fallback, provide last defined; this assumes it will override prior definitions
		return this.getUserResourceMarkdown(defs[defs.length - 1], roDoc.documentContainer.flatten(), roDoc);
	},


	tryGlobalUserResources(lowerKey: string, skipSource: ReadonlyDocument): vscode.MarkdownString {
		const possible = DocumentServices.findAllDocumentsWithCustomSymbolKey(lowerKey);
		for (let i = 0; i < possible.length; i++) {
			...
			const result = this.findGlobalUserResource(lowerKey, doc);
			if (result) {
	...
	findGlobalUserResource(lowerKey: string, roDoc: ReadonlyDocument): vscode.MarkdownString {
		const map = SymbolManager.getSymbolMap(roDoc);
		const pointers = map.collectAllSymbols().get(lowerKey);
		...
		if (defs.length === 1) {
			return this.getUserResourceMarkdown(defs[0], flatView, roDoc);
		}
	...
	breakables: ['nil', 't', '/', ')'],
	getUserResourceMarkdown(source: ISymbolReference, flatView: LispAtom[], roDoc: ReadonlyDocument): vscode.MarkdownString {
		...
		for (let i = source.flatIndex + 1; i < flatView.length; i++) {
			...
			if (this.breakables.includes(current.symbol.toLowerCase())) {
				break;
