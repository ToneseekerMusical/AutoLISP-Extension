...
const strNoADPerr: string = localize("autolispext.debug.nodap", "doesn’t exist. Verify that the file exists in the same folder as that for the product specified in the launch.json file.");
const strNoACADerr: string = localize("autolispext.debug.noacad", "doesn’t exist. Verify and correct the folder path to the product executable.");
...
class LaunchDebugAdapterExecutableFactory
	...
	createDebugAdapterDescriptor(
		...
		const lispadapterpath = ProcessPathCache.globalLispAdapterPath;
		const productStartCommand = ProcessPathCache.globalProductPath;
		...
		if (event.session && (event.session.type === launchCfgType || event.session.type === attachCfgType)) {
			if (event.event === "runtimeerror") {
				...
				*/
				...
				const msg = localize("autolispext.debug.acad.nosupport",
...
class LispLaunchConfigurationProvider implements vscode.DebugConfigurationProvider {
	...
	async resolveDebugConfiguration(folder: vscode.WorkspaceFolder | undefined, config: vscode.DebugConfiguration, token?: vscode.CancellationToken): Promise<vscode.DebugConfiguration> {
		...
		const newConfig = {} as vscode.DebugConfiguration;
		...
		if (newConfig["type"] === launchCfgType) {
			...
			if (!productPath) {
				...
				if (platform === 'Windows_NT') {
					...
					const msg = localize("autolispext.debug.prod.path.osx",
						"Specify the absolute path for the product. For example, /Applications/Autodesk/AutoCAD.app/Contents/MacOS/AutoCAD.");
					productPath = await vscode.window.showInputBox({ placeHolder: msg });
					rememberLaunchPath(productPath);
				}
				else {
					const msg = localize("autolispext.debug.prod.path.other", "Specify the absolute path for the product.");
					productPath = await vscode.window.showInputBox({ placeHolder: msg });
					rememberLaunchPath(productPath);
				}
			}
			...
			if (!existsSync(productPath)) {
				...
				const params = getExtensionSettingString(LAUNCH_PARM);
...
class LispAttachConfigurationProvider implements vscode.DebugConfigurationProvider {
	...
	async resolveDebugConfiguration(folder: vscode.WorkspaceFolder | undefined, config: vscode.DebugConfiguration, token?: vscode.CancellationToken): Promise<vscode.DebugConfiguration> {
		...
		const name = getExtensionSettingString(ATTACH_PROC);
		if (name)
			ProcessPathCache.globalAcadNameInUserAttachConfig = name;

		ProcessPathCache.clearProductProcessPathArr();
		const processId = await pickProcess(acadPid2Attach);
		if (!processId) {
			const msg = localize("autolispext.debug.noprocess.eror", "No process for which to attach could be found.");
			return vscode.window.showInformationMessage(msg).then(err => {
				console.log(err)
				return undefined;	// abort attach
			});
		}
		ProcessPathCache.chooseProductPathByPid(parseInt(processId));
		const lispadapterpath = calculateABSPathForDAP(ProcessPathCache.globalProductPath);
		if (!existsSync(lispadapterpath)) {
			vscode.window.showErrorMessage(lispadapterpath + " " + strNoADPerr);
			ProcessPathCache.globalProductPath = "";
			return undefined;
